...
describe("map warm-up state", () => {
  ...
  // `tabindex="-1"`, so Enter on it put focus inside the invisible region.
  it("hides the skip link while there is no map to skip to", () => {
    const { appMap, skipLink } = setUpPage();
    const warming = createMapWarming(appMap, { skipLink, inertSupported: true });
  ...
  describe("gating", () => {
    ...
    it("waits for an idle moment before it starts loading, and gives that up on a data tab", () => {
      const { appMap, skipLink } = setUpPage();
      let idleTask = null;
      const cancel = vi.fn();
      const warming = createMapWarming(appMap, {
