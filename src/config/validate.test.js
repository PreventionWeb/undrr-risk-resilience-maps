...
describe("validateLayers", () => {
  ...
  it("passes for a valid compound layer", () => {
    expect(() => validateLayers([makeTab("hazard", [makeCompoundLayer()])], PRIMARY)).not.toThrow();
  });
  ...
  it("throws when a tab is missing id", () => {
    expect(() => validateLayers([{ label: "x", layers: [] }], PRIMARY)).toThrow();
  });

  it("throws when a tab is missing label", () => {
    expect(() => validateLayers([{ id: "x", layers: [] }], PRIMARY)).toThrow();
  });

  it("throws when a tab layers field is not an array", () => {
    expect(() => validateLayers([{ id: "x", label: "X", layers: null }], PRIMARY)).toThrow();
  });

  // --- simple layer errors ---

  it("throws when layer is missing label", () => {
    expect(() => validateLayers([makeTab("hazard", [makeSimpleLayer({ label: "" })])], PRIMARY)).toThrow();
  });

  it("throws for an invalid layer type", () => {
    expect(() => validateLayers([makeTab("hazard", [makeSimpleLayer({ type: "xyz" })])], PRIMARY)).toThrow();
  });
  ...
  it("throws when an enabled simple layer has a null id", () => {
    expect(() => validateLayers([makeTab("hazard", [makeSimpleLayer({ id: null })])], PRIMARY)).toThrow();
  });
  ...
  it("throws on duplicate view IDs across different tabs", () => {
    ...
    expect(() =>
      validateLayers([makeTab("hazard", [layer1]), makeTab("exposure", [layer2])], PRIMARY),
    ).toThrow();
  });
  ...
  it("includes the error count in the thrown message", () => {
    ...
    expect(() => validateLayers([makeTab("hazard", layers)], PRIMARY)).toThrow(/2 layer config error/);
  });
  ...
  it("accumulates errors across multiple layers before throwing", () => {
    ...
    expect(() => validateLayers([makeTab("hazard", layers)], PRIMARY)).toThrow(/3 layer config error/);
  });
});
