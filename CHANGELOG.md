...
- "Skip to map" no longer drops you into the map while you are reading one of the information pages. The map is kept loading invisibly behind those pages, and the preview PIN gate removed the attribute that kept it out of reach, so in the session in which the PIN was entered the skip link moved focus into a region nobody could see and the next Tab jumped past the page. The map is now held out of reach for as long as it is invisible, and the skip link is hidden while there is no map to skip to.
...
- The map's loading limit is counted in real elapsed time rather than in timer ticks, which a busy browser coalesces, and the watch that counts it now stops itself instead of running for the life of the page.
