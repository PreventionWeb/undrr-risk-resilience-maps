...
router.clearAll(); // turn every layer off as one action, so it makes one entry
...
- **Subscription order.** The router subscribes to the layers store when it is created, between the sidebar's `onViewsChanged` subscriber and its row renderer. The store calls subscribers synchronously in subscription order, so the URL is written before the rows update the switches and the legend.
...
- **Failure announcements** — one visually hidden `aria-live="polite"` region per sidebar instance (`.layer-announcer.mg-u-sr-only`, no id, appended to the instance root) announces a failed load, turn-off, source switch or variant change for every kind of layer, e.g. "Could not load X. It is off." A layer with rows in several tabs is announced once, not once per row; each row still shows the message in its own `aria-hidden` `.layer-error` line. A new call for the layer clears it. Variant changes made through the external controls are announced by the controls' own status instead, and the external loading/error text in the row is visual only
...
| `src/ui/announcer.test.js`                | The live region and its rule: a duplicate for the same record dropped, the same message from a new record announced again, one record carrying a clear and a failure, and one layer's clear not wiping another's message                                                                        |
| `src/ui/sidebar.announce.test.js`         | Exactly one write to the instance's region per layer event across three placements, for the busy and failure messages; a repeated failure announced twice; the per-row `.layer-error` lines kept                                                                                                |
