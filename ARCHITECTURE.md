...
- `mg-switch` (`__input`, `__track`, `__thumb`, `__label`) — every on/off control: layer activations in home and cross-tab rows, "Show disabled", and the Sources MapX-ID switch. Mangrove 2.0's pending (`aria-busy`), `aria-disabled` and forced-colours states and its `--mg-switch-*` custom properties are used instead of local rules
