...
Static site, no backend. The app embeds MapX in an iframe via the SDK's postMessage bridge and wraps it in a sidebar UI styled with Mangrove (v2.0.0-rc.3). See [docs/product-spec.md](docs/product-spec.md) for what we're building; this doc covers how.
...
All styling builds on the [UNDRR Mangrove component library](https://assets.undrr.org/mangrove/2.0.0-rc.3/css/style.css) (v2.0.0-rc.3). Components used:
...
- `mg-switch` (`__input`, `__track`, `__thumb`, `__label`) — every on/off control: layer activations in home and cross-tab rows, "Show disabled", and the Sources MapX-ID switch. rc.3's pending (`aria-busy`), `aria-disabled` and forced-colours states and its `--mg-switch-*` custom properties are used instead of local rules
...
- `js/copy-button.js` for the site inspector's copy control, through
  ...
  has just written. rc.3 exports only `mgCopyButton`: there is no destroy, and
