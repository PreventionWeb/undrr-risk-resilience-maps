...
- **Announcements.** Each published row has its own polite live region (`.layer-announcer`, visually hidden) that announces a failed load, turn-off, source switch or variant change, and is cleared by the next call. Rows for layers without controls of their own also show the same sentence in an `aria-hidden` `.layer-error` line, so a failure is seen as well as heard. A failed variant change made through the home row's external controls is announced by those controls instead, so no row repeats it.
