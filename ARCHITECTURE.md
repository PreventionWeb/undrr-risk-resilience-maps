...
Static site, no backend. The app embeds MapX in an iframe via the SDK's postMessage bridge and wraps it in a sidebar UI styled with Mangrove (v2.0.0). See [docs/product-spec.md](docs/product-spec.md) for what we're building; this doc covers how.
...
│   │   ├── layer-panel.js      # Data tab panels (intro, groups, empty state, Show unpublished) and cross-tab sections, both stacks in an mg-accordion
...
- **Instance state.** Rows (`allRows`, `rowsByKey`), panels, "Show unpublished", the warned-keys set and the instance's live region live in the closure. The active tab, the history-entry keys and the batch depth belong to the router (see above); `sidebar.activeTab` reads `router.activeTab`.
...
All styling builds on the [UNDRR Mangrove component library](https://assets.undrr.org/mangrove/2.0.0/css/style.css) (v2.0.0). Components used:
...
- `mg-switch` (`__input`, `__track`, `__thumb`, `__label`) — every on/off control: layer activations in home and cross-tab rows, "Show unpublished", and the Sources MapX-ID switch. Mangrove 2.0's pending (`aria-busy`), `aria-disabled` and forced-colours states and its `--mg-switch-*` custom properties are used instead of local rules
...
- `mg-table--data`, `mg-table__th--sticky`, `mg-table__td--code` — the Sources tables: compact padding, subtle dividers and an uppercase header band, headers pinned while the region scrolls (which is why `.data-table-wrap` caps at 70vh — released again in `@media print`, where there is no scroll container and the cap would truncate the citations), and the MapX-ID cells in the code face. `mg-table__th--sortable` is not used: it styles a header containing a sorting button and reflects `aria-sort`, but nothing sorts the rows and the script to do so is out of scope. In rc.2 `.mg-table td, .mg-table th` at (0,1,1) defeated both `.mg-table--data` and `.mg-table__td--code` at (0,1,0), which rc.3 resolved in the library stylesheet (`.mg-table.mg-table--data` and `.mg-table td.mg-table__td--code`). `--small` and the `--data` header band are written as (0,2,1) and do work
...
- `js/copy-button.js` for the site inspector's copy control, through
  ...
  has just written. Mangrove 2.0 exports only `mgCopyButton`: there is no destroy, and
...
- **Show unpublished toggle** — reveals unpublished review-only layer entries in the current category without making them toggleable on the map
...
| `src/ui/layer-panel.test.js`              | Tab panel intro/empty state/groups, unpublished rows and Show unpublished visibility, cross-tab sections (skipped tabs, groups, compact rows)                                                                                                                                                                                                                                                                                                           |
